...
function initMultiplayerGame(
  controllerFunction: (gameId: string, playerName: string) => Promise<Response>,
  errorMessagePrefix: string,
  successfulMessage: string
) {
  const form = document.getElementById(
    "start-multiplayer-game-form"
  ) as HTMLFormElement;
  ...
  if (!form.checkValidity()) {
    ...
    controllerFunction(lobbyId, playerName).then((response) => {
      const responseDiv = document.getElementById(
        "multiplayer-game-init-error-response-div"
      );
...
function preInitializeListener(gameId: string, playerId: string): Socket {
  ...
  registerListener(socket, PlayerJoin.name, (eventData: PlayerJoin) => {
  ...
  registerListener(socket, PlayerLeft.name, (eventData: PlayerLeft) => {
    if (gameId === eventData.game_id) {
  ...
  registerListener(socket, GameStarting.name, (eventData: GameStarting) => {
    console.log("Received game starting event...");
    document.getElementById("lobby-info-div").innerText = eventData.message;
  });
  registerListener(socket, GameStarted.name, (evenData: GameStarted) => {
  ...
  registerListener(
    ...
    (eventData: UpdatePlayersData) => {
      if (eventData.game_id === gameId) {
        console.log(
          `Received relevant update players data event ${JSON.stringify(
            eventData
          )}`
        );
  ...
  registerListener(socket, GameEnded.name, (eventData: GameEnded) => {
    ...
    winnerDiv.innerHTML = `${eventData.players_ranked[0].player_id} wins!`;
    ...
    eventData.players_ranked.forEach((player) => {
      ...
      const suffix = player.current_score === 1 ? "point" : "points";
      div.innerText = `${player.player_id}: ${player.current_score} ${suffix}`;
...
function updateLobby(gameId: string, hasAllPlayers: object) {
  ...
  lobbyGameIdHeader.innerText = `lobby name: ${gameId}`;
...
function updateThisPlayerDiv(
  socket: Socket,
  gameId: string,
  pData: PlayerData
) {
  singlePlayerController
    .getChallengeImage(pData.current_image_id)
    .then((blob) => {
      const imageElement = document.getElementById(
        "this-player-current-challenge-img"
      ) as HTMLImageElement;
      imageElement.src = urlCreator.createObjectURL(blob);
    });

  document.getElementById("this-player-score-span").textContent =
    pData.current_score?.toString();
